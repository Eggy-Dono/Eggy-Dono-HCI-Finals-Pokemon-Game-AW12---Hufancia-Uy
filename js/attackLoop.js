  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (!crit) {
      await game.menu.drawCommentMenu(
        `${game.playerPokemon.name} used ${attack.name} and makes damage of ${attack.damage} hp!`
      );
    } else {
      await game.menu.drawCommentMenu(
        `${game.playerPokemon.name} used ${attack.name} and makes CRITICAL damage of ${attack.damage} hp!`,
        3000
      );
    }
...
const opponentAttacks = async () => {
  ...
  if (!crit) {
    await game.menu.drawCommentMenu(
      `${game.opponentPokemon.name} used ${attack.name} and makes damage of ${attack.damage} hp!`
    );
  } else {
    await game.menu.drawCommentMenu(
      `${game.opponentPokemon.name} used ${attack.name} and makes CRITICAL damage of ${attack.damage} hp!`,
      3000
    );
  }
